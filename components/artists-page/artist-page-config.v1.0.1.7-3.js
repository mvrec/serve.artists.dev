...
<div id="profile-dp" class="relative shrink-0 w-[200px] h-[200px] group [perspective:1000px] cursor-pointer">
    <div class="relative w-full h-full transition-transform duration-700 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)]">
        ...
        <div class="absolute inset-0 [backface-visibility:hidden]">
            <img src="${pImageSrc}" alt="${pImageAlt}" class="w-full h-full rounded-2xl object-cover artist-pick shadow-xl">
    ...
    try {
      ...
      for (const item of data.items) {
        ...
        <div class="w-full aspect-video rounded-lg overflow-hidden relative shadow-xl mb-3 bg-neutral-800 flex items-center justify-center">
          ...
          ${
            isShort
              ? `<span class="absolute top-2 left-2 bg-red-600/90 text-white text-xs font-medium px-2 py-0.5 rounded">SHORTS</span><svg class="absolute w-10 h-10 text-brand-500 opacity-80 group-hover:opacity-100 transition duration-300" viewBox="0 0 98.94 122.88" style="enable-background:new 0 0 98.94 122.88">
  ...
  function pinnedStatus() {
    ...
    if (targetSpan) {
      let pinId = (targetSpan.getAttribute("data-pin-track") || "").trim();
      let embedType = "track";
      if (!pinId) {
        pinId = (targetSpan.getAttribute("data-pin-album") || "").trim();
        embedType = "album";
      }
      if (pinId) {
